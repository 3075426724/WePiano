...
class AdminEnrollService extends BaseProjectAdminService {
	...
	async vouchEnrollSetup(id, vouch) {
		this.AppError('[琴房]该功能暂不开放，如有需要请加作者微信：cclinux0730');
	}
	...
	async getEnrollJoinList({
		...
		if (util.isDefined(search) && search.includes('#')) {
			let start = search.split('#')[0];
			let end = search.split('#')[1];
			where.ENROLL_JOIN_DAY = ['between', start, end];
		}
